...
/*
  MP3 "best effort":
  För att hålla detta robust och kompilerbart på ESP32-C3 utan extra tunga beroenden
  är MP3-dekodning avstängd som standard. URL/local .mp3 kommer normalt falla tillbaka
  till WAV.
*/
extern "C" {
  typedef struct {
  ...
  static void mp3dec_init(mp3dec_t*) {}
  static int mp3dec_decode_frame(mp3dec_t*, const uint8_t*, int, int16_t*, mp3dec_frame_info_t* info) {
    info->frame_bytes = 0;
    info->channels = 0;
    info->hz = 0;
    info->layer = 0;
...
// 200 kHz @ 8 bit är för högt för ESP32-C3 LEDC; sänk till en stabil nivå.
...
public:
  ...
  bool playLocal(const String& path, uint8_t vol) {
    ...
    String p = path;
    if (!p.startsWith("/")) p = "/" + p;
    if (!fileExists(p.c_str())) { lastErr = "file_not_found"; return false; }

    file = LittleFS.open(p, "r");
    if (!file) { lastErr = "file_open_failed"; return false; }
  ...
  bool playUrl(const String& url, uint8_t vol) {
    ...
    bool secure = startsWithIgnoreCase(url, "https://");
    if (secure) {
...
private:
  ...
  int audioPin = DEFAULT_AUDIO_PWM_PIN;
  int sampleRate = SR_16000;
  uint8_t volume = 80;
  ...
  static void timerThunk(void* arg) { static_cast<AudioPlayer*>(arg)->onTick(); }
  ...
  void rbReset() { rbHead = rbTail = rbCount = 0; }
  ...
  bool fillMp3() {
    // Stub decoder: kommer normalt inte ge ljud. Förväntat beteende är fallback WAV.
    if (mp3InFilled < 1024) {
      int r = 0;
      if (file) {
    ...
    int samples = mp3dec_decode_frame(&mp3dec, mp3In, mp3InFilled, pcm, &info);

    if (info.frame_bytes > 0) {
      int remain = mp3InFilled - info.frame_bytes;
      if (remain > 0) memmove(mp3In, mp3In + info.frame_bytes, remain);
      mp3InFilled = remain;

      if (info.hz == SR_11025 || info.hz == SR_16000 || info.hz == SR_22050) setSampleRate(info.hz);

      int ch = max(1, info.channels);
      for (int i = 0; i < samples; i++) {
        int16_t s = pcm[i * ch];
        if (ch == 2) {
          int16_t r = pcm[i * ch + 1];
          s = (int16_t)(((int32_t)s + (int32_t)r) / 2);
        }
        if (!rbPush(s)) break;
      }
      return true;
...
static bool playAlarmAudioWithFallback(const AlarmConfig& a) {
  lastAudioError = "";
  ...
  if (a.audio_type == AUDIO_URL) {
    ...
    if (!ok) {
      ...
      if (!ok && fileExists("/audio/default.wav")) ok = tryLocal("/audio/default.wav");
    }
  } else {
